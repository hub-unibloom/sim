...
export const user = pgTable('user', {
  ...
  interactionRhythmMs: integer('interaction_rhythm_ms').default(0),
  lastInteraction: timestamp('last_interaction'),
  plano: text('plano').default('free'),
  emotionalHomeostasis: vector('emotional_homeostasis', { dimensions: 9 }),
})
...
export const memories = pgTable('memories', {
  id: text('id').primaryKey(),
  userUuid: text('user_uuid').references(() => user.id, { onDelete: 'cascade' }).notNull(),
  semanticText: text('semantic_text').notNull(),
  timestamp: timestamp('timestamp').notNull(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  emotionalHomeostasis: jsonb('emotional_homeostasis'),
...
export const graphNodes = pgTable('graph_nodes', {
  ...
  type: text('type').notNull(),
  mass: doublePrecision('mass').default(1.0),
  activationEnergy: doublePrecision('activation_energy').default(0.0),
...
export const graphEdges = pgTable('graph_edges', {
  id: text('id').primaryKey(),
  sourceUuid: text('source_uuid').references(() => graphNodes.uuid, { onDelete: 'cascade' }).notNull(),
  targetUuid: text('target_uuid').references(() => graphNodes.uuid, { onDelete: 'cascade' }).notNull(),
  weight: doublePrecision('weight').default(0.0),
  type: text('type').notNull()
});


export const memoriesNodes = pgTable('memories_nodes', {
  id: text('id').primaryKey(),
  memoryId: text('memory_id').references(() => memories.id, { onDelete: 'cascade' }).notNull(),
  nodeId: text('node_id').references(() => graphNodes.uuid, { onDelete: 'cascade' }).notNull()
}, (table) => ({
  memoryNodeUnique: uniqueIndex('memories_nodes_memory_node_unique').on(table.memoryId, table.nodeId)
}));
