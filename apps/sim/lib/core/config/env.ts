...
export const env = createEnv({
  ...
  server: {
    ...
    CASCATA_POSTGRES_URL: z.string().url().optional(),            // External Cascata PostgreSQL (via PgBouncer)
    CASCATA_QDRANT_URL: z.string().url().optional(),            // External Qdrant vector DB URL
    CASCATA_QDRANT_API_KEY: z.string().optional(),                  // Qdrant API key for authentication
    CASCATA_REDIS_URL: z.string().url().optional(),            // External Redis for Thalamus cache
    CHESHIRE_EMBEDDING_MODEL: z.string().optional().default('text-embedding-3-small'), // Embedding model
    CHESHIRE_EMBEDDING_DIM: z.coerce.number().optional().default(1536), // Vector dimensions
    CHESHIRE_LLM_MODEL: z.string().optional().default('gpt-4-turbo'), // LLM for biosynthesis
    N8N_WEBHOOK_URL: z.string().url().optional(),            // n8n webhook for proactive triggers (Kairos/Silence)
  },
